...
function draw() { //let's draw it tho!
  ...
  circleArr.forEach(function (circle) {
    fill(circle.color.r, circle.color.g, circle.color.b); //apply rgb value to fill
    stroke(0); //keep the stroke black
    bezier(circle.size.x, circle.size.y, circle.size.x*2, circle.size.y*2, circle.size.xRadius, circle.size.yRadius, circle.size.xRadius*2, circle.size.yRadius*2); //draw an ellipse with my modified volume value
  });
...
function createCircle() {
    ...
    var circleX = constrain(vol*100, 0, width);
    var circleY = constrain(vol^2, 0, height);
